...
locals {
  ...
  cross_region_vpc_id             = var.create_vpc == true ? module.shared_vpc_cross_region[0].vpc_id : var.cross_region_vpc_id
  ...
  cross_region_public_subnet_ids  = var.create_vpc == true && var.deploy_cross_region_read_replica ? module.shared_vpc_cross_region[0].public_subnet_ids : var.cross_region_public_subnet_ids
  cross_region_private_subnet_ids = var.create_vpc == true && var.deploy_cross_region_read_replica ? module.shared_vpc_cross_region[0].private_subnet_ids : var.cross_region_private_subnet_ids
  database_az                     = var.create_vpc == true ? module.shared_vpc[0].az_1 : data.aws_availability_zones.available.names[0]
  cross_region_database_az        = var.create_vpc == true ? module.shared_vpc_cross_region[0].az_1 : data.aws_availability_zones.available_cross_region.names[0]
